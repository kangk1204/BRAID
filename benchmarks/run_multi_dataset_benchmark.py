#!/usr/bin/env python3
"""Multi-dataset benchmark for BRAID.

Aligns FASTQ files with HISAT2, runs BRAID, evaluates with GFFcompare against
GENCODE v38, and generates a combined benchmark report.
...
def generate_report(all_results: dict[str, list[dict]]) -> None:
    ...
    with PdfPages(str(report_path)) as pdf:
        ...
        if datasets:
            ...
            for ds in datasets:
                rs_val = next(
                    (
                        ...
                        if "BRAID" in r.get("tool", "")
                    ),
                    0,
                )
                rs_intron_pr.append(rs_val)

            ax.bar(list(x), rs_intron_pr, label="BRAID", color="#2ecc71")
            ...
            ax.bar(list(x), rs_exact, label="BRAID", color="#2ecc71")
...
def process_dataset(ds: DatasetConfig, threads: int = 8) -> list[dict] | None:
    ...
    if not Path(ds.bam_path).exists():
        print("  BAM not found, aligning from FASTQ...")
