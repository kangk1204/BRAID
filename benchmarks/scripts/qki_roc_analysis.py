...
# Resolve paths relative to the repo root so this runs from a clean clone
# (previously hardcoded to a developer's 24_BRAID tree).
_REPO_ROOT = Path(__file__).resolve().parents[2]

# ── Load data ────────────────────────────────────────────────────────────────
with open(_REPO_ROOT / 'benchmarks/results/qki_rmats_benchmark_results.json') as f:
    data = json.load(f)
...
print(
    f"Validated: {len(data['validated'])} total, {len(validated_matched)} matched, "
    f"{len(validated_unmatched)} unmatched"
)
print(
    f"Failed:    {len(data['failed'])} total, {len(failed_matched)} matched, "
    f"{len(failed_unmatched)} unmatched"
)
...
for name, scores in methods.items():
    ...
    pr_results[name] = {
        'precision': precision, 'recall': recall, 'thresholds': thresholds, 'auc': pr_auc,
    }
...
threshold_table = []
print(
    f"\n  {'Filter':<30s} {'TP':>4s} {'FP':>4s} {'FN':>4s} {'TN':>4s} "
    f"{'Prec':>7s} {'Recall':>7s} {'F1':>7s} {'FPR':>7s}"
)
print("  " + "-"*90)
for name, fn in filters.items():
    ...
    print(
        f"  {name:<30s} {tp:4d} {fp:4d} {fn_count:4d} {tn:4d} "
        f"{prec:7.3f} {recall:7.3f} {f1:7.3f} {fpr_val:7.3f}"
    )
...
for row in threshold_table:
    ...
    nnv_results.append(
        {'filter': row['filter'], 'nnv': round(nnv, 4), 'calls': tp + fp, 'true_positives': tp}
    )
...
out_path = _REPO_ROOT / 'benchmarks/results/qki_roc_analysis.json'
