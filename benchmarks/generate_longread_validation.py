"""Generate long-read cross-validation report for BRAID versions.

Uses ENCODE PacBio Iso-Seq K562 data (ENCSR589FUJ, rep1 ENCFF652QLH) as
independent validation reference. Compares BRAID short-read assembler outputs
against long-read transcript annotations to verify assembly accuracy without
reliance on GENCODE annotation.
"""
...
def main() -> None:
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        fig3.suptitle(
            "IntronPr: GENCODE Reference vs Long-Read Reference",
            fontsize=14,
            fontweight="bold",
        )
        ...
        analysis += (
            f"   -> v10c change vs v8c: "
            f"{rs10.get('IntronPr', 0) - rs8.get('IntronPr', 0):+.1f}pp\n\n"
        )
        ...
        analysis += "   -> Lower is better for false junction control\n\n"
        ...
        analysis += "   -> Higher exact matches indicate better long-read agreement\n\n"

        analysis += "4. CONSISTENCY CHECK:\n"
        analysis += "   Results vs GENCODE and vs Long-read test the same BRAID\n"
        analysis += "   versions against independent references.\n\n"

        analysis += "5. CONCLUSION:\n"
        analysis += "   Long-read validation is retained as a BRAID-internal\n"
        analysis += "   assembly sanity check without external assembler baselines.\n"
