    ...
    *,
    ...
    for label in ordered_labels:
        ...
        support_bin_factors[label] = {
            ...
            "calibrated_median_ci_width":
                calibrated_support_bin_summary[label].get("median_ci_width"),
        }
    ...
    return {
        ...
        "raw_overall_coverage": float(
            np.mean([_contains_after_inflation(row, 1.0) for row in all_rows])
        )
        if all_rows
        else None,
...
def _effect_gate_for_rows(
    ...
    *,
    ...
    def _consider(
        selected: list[dict[str, object]], *, effect_threshold: float,
        snr_threshold: float, cv_threshold: float | None,
    ) -> None:
        nonlocal best
        if len(selected) < min_confident_events:
            return
...
def benchmark_pacbio_junction_validation(
    *,
    ...
    # Paths are env-overridable so the validation can be re-run from the
    # regenerate_pacbio_validation.py driver outputs (chr-named GENCODE) rather than
    # the original real_benchmark/ layout. BAM, annotation, and long-read GTF must use
    # a consistent chromosome-naming convention.
    GTF = os.environ.get("BRAID_PACBIO_GTF", "real_benchmark/annotation/gencode.v38.nochr.gtf")
    BAM = os.environ.get("BRAID_PACBIO_BAM", "real_benchmark/bam/SRR387661.bam")
    LR_GTF = os.environ.get("BRAID_PACBIO_LR_GTF", "real_benchmark/longread/ENCFF652QLH.nochr.gtf")
    ...
    try:
        ...
        return {
            ...
            "n_events": n_events,
            # Minimal per-event records (commit-safe derived values) so the
            # deconfounded sharpness analysis can run on the real fixture.
            "per_event": [
                {
                    "gene": str(row["gene"]),
                    "event_id": str(row["event_id"]),
                    "psi_hat": float(row["braid_psi"]),
                    "lr_psi": float(row["lr_psi"]),
                    "support": float(row["total_support"]),
                    "ci_low": float(row["ci_low"]),
                    "ci_high": float(row["ci_high"]),
                }
                for row in native_rows
