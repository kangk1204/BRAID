#!/usr/bin/env python3
# ruff: noqa: E501
# (cut-section QKI reporting script; long formatted print/report lines kept inline)
...
# Opt-in conformal calibrator for the per-event PSI CI. None = legacy inflation,
# which preserves the committed benchmark numbers; set via --conformal in main().
...
def _build_condition_metrics(
    ...
    *,
    ...
    psi, ci_low, ci_high, cv = bootstrap_psi(
        ...
        conformal_calibrator=_CONFORMAL_CALIBRATOR,
    )
...
def _build_matched_null_control_rows(
    *,
    ...
    def priority(event_id):
        return _stable_seed(seed, cohort_name, event_id)
...
def _write_json(output_path: str, payload: dict) -> None:
    """Write one JSON payload (NaN floats coerced to null for strict JSON)."""
    def _safe(o):
    ...
    with open(output_path, "w", encoding="utf-8") as handle:
        json.dump(_safe(payload), handle, indent=2, allow_nan=False)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--conformal",
        action="store_true",
        help="Use the conformal calibrator for per-event PSI CIs (default: legacy inflation).",
    )
...
def main() -> None:
    ...
    if getattr(args, "conformal", False):
        ...
        _CONFORMAL_CALIBRATOR = load_default_conformal_calibrator()
        print("Using conformal calibrator for per-event PSI CIs.")
