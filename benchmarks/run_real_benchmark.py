...
from benchmarks.run_benchmark import resolve_assembled_annotation_path  # noqa: E402
...
def count_transcripts(gtf_path: str) -> int:
    ...
    resolved_gtf = resolve_assembled_annotation_path(gtf_path)
    count = 0
    with open(resolved_gtf) as f:
        for line in f:
            if line.startswith("#"):
                continue
            parts = line.split("\t")
...
def run_benchmark(config: RealBenchmarkConfig) -> dict:
    ...
    rs_gtf_resolved = resolve_assembled_annotation_path(rs_gtf)
    if rs_gtf_resolved.exists():
        rs_n_tx = count_transcripts(str(rs_gtf_resolved))
        rs_prefix = str(out_dir / f"{sample}_braid_gffcmp")
        rs_metrics = run_gffcompare(str(rs_gtf_resolved), str(annotation_gtf), rs_prefix)
        results["tools"]["BRAID"] = {
            "gtf_path": str(rs_gtf_resolved),
...
def main() -> None:
    ...
    parser.add_argument(
        "--engine", choices=["legacy", "iterative_v2", "sota", "braid_v2"], default="braid_v2",
        help="BRAID engine to use (default: braid_v2).",
    )
    parser.add_argument(
        "--decomposer", dest="engine", choices=["legacy", "iterative_v2", "sota", "braid_v2"],
