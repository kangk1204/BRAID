...
Downloads or generates synthetic RNA-seq data, runs BRAID and optional baseline
tools on the same input, and evaluates assembly quality using
...
class SyntheticDataGenerator:
    ...
    def _generate_reads_for_transcript(
        ...
        for read_idx in range(n_reads):
            ...
            # Add conventional strand tags for spliced reads.
...
def resolve_assembled_annotation_path(output_path: str | Path) -> Path:
    """Resolve the actual annotation file emitted by ``braid assemble``.

    Recent BRAID CLI builds materialize single-sample output as a directory
    containing ``<sample>.gtf`` plus ``summary.tsv``. Older benchmark runners
    assumed ``-o`` always pointed at a direct GTF file. Accept both forms.
    """
    ...
    candidates = sorted(
        child for child in path.iterdir() if child.is_file() and child.suffix in {".gtf", ".gff3"}
    )
    if len(candidates) == 1:
        return candidates[0]
    if len(candidates) > 1:
        preferred = [child for child in candidates if child.stem != "summary"]
        if len(preferred) == 1:
            return preferred[0]
...
class BenchmarkRunner:
    ...
    def run_braid(
        ...
        resolved_gtf_path = resolve_assembled_annotation_path(gtf_path)
        return str(resolved_gtf_path), elapsed, peak_mb
    ...
    def evaluate_gtf(
        ...
        """
        pred_tx = _parse_gtf_transcripts(str(resolve_assembled_annotation_path(predicted_gtf)))
