...
from benchmarks.run_benchmark import (
    ...
    resolve_assembled_annotation_path,
)
        ...
        if name == "legacy":
            ...
            variants.append(
                VariantSpec(
                    ...
                    decomposer="braid_v2",
                    builder_profile="default",
                    shadow_decomposer=shadow_mode if shadow_mode != "braid_v2" else None,
                )
            )
        elif name == "braid_v2_no_motif_validation":
            variants.append(
                VariantSpec(
                    name="braid_v2_no_motif_validation",
                    decomposer="braid_v2",
                    builder_profile="default",
                    enable_motif_validation=False,
                    shadow_decomposer=shadow_mode if shadow_mode != "braid_v2" else None,
...
def _build_assemble_command(
    ...
    *,
    ...
    if variant.shadow_decomposer:
        cmd.extend(["--shadow-engine", variant.shadow_decomposer])
...
def _build_variant_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        "command": command,
        "gtf_path": str(resolved_gtf_path.resolve()),
    ...
    if row["status"] == "ok" and resolved_gtf_path.exists():
        row["transcript_count"] = _count_transcripts(resolved_gtf_path)
        row["metrics"] = {
            key: _round_metric(value)
            for key, value in evaluator.evaluate_gtf(
                str(resolved_gtf_path), str(truth_gtf)
            ).items()
        }
    else:
        row["partial_output_present"] = resolved_gtf_path.exists()
    ...
    loss_accounting = _build_loss_accounting(
        truth_gtf, resolved_gtf_path, diagnostics_dir
    )
    if loss_accounting:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--shadow-decomposer",
        choices=["legacy", "iterative_v2", "braid_v2"],
        default=None,
