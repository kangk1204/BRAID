...
def main() -> None:
    ...
    # Load data
    from rapidsplice.dashboard.data_loader import load_events, load_gtf_transcripts
    ...
    # Page dispatch
    if page == "Overview":
        from rapidsplice.dashboard.components.summary import render_summary
        render_summary(events_df, transcripts_df)

    elif page == "Gene Browser":
        from rapidsplice.dashboard.components.gene_browser import render_gene_browser
        render_gene_browser(events_df, transcripts_df)

    elif page == "Sashimi Plots":
        from rapidsplice.dashboard.components.sashimi import render_sashimi
        render_sashimi(events_df, transcripts_df, bam_path=args.bam)

    elif page == "Event Explorer":
        from rapidsplice.dashboard.components.event_table import render_event_table
        render_event_table(events_df)

    elif page == "PSI Analysis":
        from rapidsplice.dashboard.components.psi_plots import render_psi_analysis
