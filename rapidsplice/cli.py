"""Command-line interface for BRAID with a legacy ``rapidsplice`` alias."""
...
from rapidsplice.pipeline import AssemblyPipeline, PipelineConfig
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "BRAID: RNA-seq splicing confidence and assembly toolkit.  "
            "Public command name is `braid`; the legacy `rapidsplice` alias "
...
def _run_analyze(args: argparse.Namespace) -> None:
    ...
    from rapidsplice.splicing.io import write_events_tsv, write_ioe
...
def _run_dashboard(args: argparse.Namespace) -> None:
    ...
    app_path = str(
        importlib.resources.files("rapidsplice.dashboard") / "app.py"
    )
...
def _run_target(args: argparse.Namespace) -> None:
    ...
    Args:
        args: Parsed CLI arguments.
    """
    from rapidsplice.target.assembler import (
        TargetConfig,
        assemble_target,
        format_target_report,
    )
    from rapidsplice.target.extractor import lookup_gene, parse_region_string
    ...
    if output_format == "text":
        ...
        from rapidsplice.io.gtf_writer import GtfWriter, TranscriptRecord
        records = []
...
def _run_fastq_target(args: argparse.Namespace) -> None:
    ...
    Args:
        args: Parsed CLI arguments.
    """
    from rapidsplice.target.fastq_pipeline import (
        FastqTargetConfig,
...
def _run_doctor(args: argparse.Namespace) -> None:
    ...
    Args:
        args: Parsed CLI arguments.
    """
    from rapidsplice.doctor import report_install_check
...
def main() -> None:
    ...
    This is the main entry point invoked by the ``rapidsplice`` console
