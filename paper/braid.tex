...
On the QKI knockdown RT-PCR benchmark, the rMATS + BRAID hybrid workflow classifies 10 of 80 validated events as high-confidence, with zero false positives on both 25 experimentally confirmed negatives and 80 matched null controls.
...
This dataset provides three independent cohorts: 80 RT-PCR-validated exon-skipping events (\textit{validated}), 25 events that were tested by RT-PCR but showed no splicing change (\textit{RT-PCR failed}), and 80 computationally selected non-differential events with matched read support (\textit{matched null}).
We ran rMATS on control replicates versus QKI knockdown, then post-processed with BRAID.

Among the 44 events called significant by rMATS, BRAID classified 14 as \textit{supported} and 10 as \textit{high-confidence} (Table~\ref{tab:qki}, Figure~\ref{fig:qki}).
Critically, \textbf{zero} events from either negative cohort---the 25 experimentally confirmed negatives and the 80 computationally matched nulls---were classified as supported or high-confidence at any tier.
The experimental negative cohort (RT-PCR failed) is particularly informative because these events were actually tested by RT-PCR and found to show no differential splicing, providing independent \textit{experimental} rather than \textit{computational} evidence that BRAID does not generate false confident calls.

\begin{table}[htbp]
\centering
\caption{QKI RT-PCR benchmark across three independent cohorts. Tiers are nested subsets: 44 rMATS significant $\supset$ 14 supported $\supset$ 10 high-confidence. The RT-PCR Failed cohort provides \textit{experimental} (not computational) evidence of zero false positives.}
\label{tab:qki}
\begin{tabular}{lcccc}
\toprule
 & Validated & RT-PCR Failed & Matched Null \\
Tier & ($n=80$) & ($n=25$) & ($n=80$) \\
\midrule
rMATS significant & 44 (55\%) & 0 (0\%) & 0 (0\%) \\
\quad + BRAID supported & 14 (18\%) & 0 (0\%) & 0 (0\%) \\
\quad\quad + BRAID high-confidence & 10 (13\%) & 0 (0\%) & 0 (0\%) \\
\bottomrule
\end{tabular}
\end{table}
