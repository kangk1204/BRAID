...
On 204 A3SS/A5SS events across 15 genes, BRAID achieves 94.6\% CI coverage at a nominal 95\% level ($r = 0.831$ versus long-read PSI), compared to 72.5\% without support-adaptive calibration.
On the QKI knockdown RT-PCR benchmark, rMATS calls 17 of 25 experimentally confirmed negatives as significant (FDR $< 0.05$); BRAID's confidence tiers classify zero of these as supported or high-confidence.
Consistent results are observed on two additional datasets: TRA2 knockdown (0 false positives among 40,877 rMATS non-significant SE events) and TARDBP knockdown (0 among 82,456 non-significant events across five event types).
...
BRAID's support-adaptive scaling---where the scale parameter $\lambda$ is learned per support bin via leave-one-gene-out cross-validation---increases coverage to 94.6\% while maintaining identical point estimates ($r = 0.831$, $R^2 = 0.691$).
The uncalibrated model produces 33 confident calls with 3 errors (90.9\% accuracy); BRAID produces 10 confident calls with zero errors (100\%).
...
In TARDBP knockdown in K562 cells (ENCODE; 84,838 events across five AS types; hg38 coordinates), BRAID classified 1,120 of 2,382 significant events as supported and 23 as high-confidence, again with zero false positives among 82,456 non-significant events.

We note that these non-significant pools are \textit{computationally defined} (rMATS FDR $\geq 0.05$), not experimentally confirmed negatives like the QKI RT-PCR failed cohort.
They demonstrate consistency of BRAID's conservative tiering across cell lines (HeLa, MDA-MB-231, K562), knockdown targets (QKI, TRA2, TARDBP), event types (SE, A3SS, A5SS, MXE, RI), and genome builds (hg19, hg38), but do not constitute independent experimental validation.
