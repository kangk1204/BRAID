...
BRAID reduces this to 16\% (4/25 at the high-confidence tier) while retaining 10/80 validated events---outperforming simple FDR threshold tightening (FDR~$< 0.0001$: 32\% FPR, 24/80 recall).
...
The four false positives (CLSTN1, ESYT2, FN1, SMARCA1) have very low rMATS FDR ($< 0.05$), large $|\Delta\Psi|$, and sufficient read support; whether these represent true RT-PCR assay failures or genuine BRAID errors remains an open question.

\begin{table}[htbp]
\centering
\caption{QKI RT-PCR benchmark. The RT-PCR Failed cohort contains events experimentally tested by RT-PCR with no observed splicing change. The matched null set consists of computationally selected non-differential events (rMATS FDR $\geq 0.5$, support-matched). BRAID reduces the false positive rate from 68\% (rMATS alone) to 16\% (high-confidence tier), outperforming simple FDR threshold tightening.}
\label{tab:qki}
\begin{tabular}{lcccc}
\toprule
...
BRAID's confidence tiers reduce the false positive rate from 68\% to 16\% while retaining 10 high-confidence true positives---a better FPR-recall tradeoff than simple FDR threshold tightening (FDR $< 0.0001$: 32\% FPR, 24 true positives).
...
BRAID's confidence tiering is conservative: only 10/80 validated events pass the high-confidence tier (12.5\% recall), with 4/25 false positives on RT-PCR negatives (16\% FPR).
...
TARDBP knockdown: ENCODE ENCSR905HLE (K562; hg38).
