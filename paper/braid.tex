...
On the QKI knockdown RT-PCR benchmark, rMATS alone calls 17 of 25 experimentally confirmed negatives as significant (FDR $< 0.05$), whereas BRAID's high-confidence tier retains 10 validated events with zero false positives across both 25 experimental negatives and 80 matched null controls.
...
The RT-PCR Failed cohort reveals a striking result: rMATS calls 17 of 25 experimentally confirmed negatives as significant (FDR $< 0.05$; median FDR $= 0.002$, median $|\Delta\Psi| = 0.42$).
These are events that passed rMATS's statistical threshold yet show no splicing change by RT-PCR---a 68\% false positive rate among the experimental negatives.
In contrast, BRAID classified \textbf{zero} of these 25 events as supported or high-confidence.
This demonstrates that BRAID's calibrated confidence tiers provide a safeguard against false positives that $p$-value thresholds alone cannot offer.

\begin{table}[htbp]
\centering
\caption{QKI RT-PCR benchmark. The RT-PCR Failed cohort contains events experimentally tested by RT-PCR with \textit{no} observed splicing change. Notably, rMATS calls 17 of these 25 negatives as significant (FDR $< 0.05$, median FDR $= 0.002$), whereas BRAID classifies zero as supported or high-confidence.}
\label{tab:qki}
\begin{tabular}{lccc}
\toprule
...
Tier & ($n=80$) & ($n=25$) & ($n=80$) \\
\midrule
rMATS FDR $< 0.05$ & 44 (55\%) & \textbf{17 (68\%)} & 0 (0\%) \\
\quad + BRAID supported & 14 (18\%) & \textbf{0 (0\%)} & 0 (0\%) \\
\quad\quad + BRAID high-confidence & 10 (13\%) & \textbf{0 (0\%)} & 0 (0\%) \\
\bottomrule
\end{tabular}
\end{table}
...
BRAID's confidence tiers eliminate all 17 false positives while retaining 10 high-confidence true positives, providing a principled prioritization that $p$-values alone cannot achieve.
