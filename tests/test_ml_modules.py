...
class TestGNNModel:
    ...
    def test_gnn_scorer_uses_fallback_fn(self) -> None:
        from braid.scoring.gnn_model import GNNScorer, GraphData

        seen: dict[str, np.ndarray] = {}

        def fallback(features: np.ndarray) -> float:
            seen["features"] = features
            return 0.42

        scorer = GNNScorer(fallback_fn=fallback)
        gd = GraphData(
            node_features=np.ones((3, 8), dtype=np.float32),
            edge_index=np.empty((2, 0), dtype=np.int64),
            edge_features=np.empty((0, 6), dtype=np.float32),
            path_node_mask=np.array([1, 1, 0], dtype=np.float32),
        )

        assert scorer.score(gd) == pytest.approx(0.42)
        assert seen["features"].shape == (8,)

    def test_splice_graph_to_features_accepts_empty_csr(self) -> None:
        from braid.graph.splice_graph import CSRGraph, SpliceGraph
        from braid.scoring.gnn_model import splice_graph_to_features
        ...
        graph = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=0)

        data = splice_graph_to_features(csr, graph)

        assert data.node_features.shape == (0, 8)
        assert data.edge_features.shape == (0, 6)
        assert data.path_node_mask.shape == (0,)
...
class TestAutoFilter:
    ...
    def test_evaluate_filter_config_supervised_uses_intron_chains(self) -> None:
        """Terminal-boundary shifts should not deflate intron-chain F1."""
        from braid.flow.decompose import Transcript
        from braid.scoring.auto_filter import evaluate_filter_config
        from braid.scoring.features import TranscriptFeatures
        from braid.scoring.filter import FilterConfig
...
class TestNeuralPSI:
    ...
    @_REQUIRES_WORKING_TORCH
    def test_neural_psi_trained_interval_is_deterministic(self) -> None:
        from braid.splicing.neural_psi import NeuralPSIEstimator

        rng = np.random.default_rng(0)
        estimator = NeuralPSIEstimator()
        features = rng.random((12, 8), dtype=np.float32)
        targets = rng.random(12, dtype=np.float32)
        estimator.train_model(features, targets, n_epochs=1, lr=1e-3)

        first = estimator.estimate(30, 10)
        second = estimator.estimate(30, 10)
...
class TestBoundaryDetector:
    ...
    def test_boundary_window_size_must_support_gradient(self) -> None:
        from braid.graph.boundary_detector import (
            BoundaryDetector,
            extract_boundary_features,
        )
        ...
        with pytest.raises(ValueError, match="window_size"):
            extract_boundary_features(np.ones(10, dtype=np.float32), 5, 1)
        with pytest.raises(ValueError, match="window_size"):
            BoundaryDetector(window_size=1)
