...
class TestEventsTSV:
    ...
    def test_formula_injection_neutralised(self, tmp_path: Path) -> None:
        """A malicious gene_id/event_id starting with a formula character must be
        written as inert text (spreadsheet formula-injection hardening)."""
        events = [
            ASEvent(
                event_id="=cmd",
                event_type=EventType.SE,
                ...
                chrom="chr1",
                strand="+",
                coordinates={"upstream_exon_end": 200},
                inclusion_transcripts=["tx1"],
                exclusion_transcripts=["tx2"],
        ...
        psi_results = [
            PSIResult("=cmd", 0.75, 75, 25, 2, 1, 100, 0.65, 0.85),
        ]
        tsv_path = tmp_path / "evil.tsv"
        write_events_tsv(tsv_path, events, psi_results, [0.9])
        cells = tsv_path.read_text().splitlines()[1].split("\t")
        assert cells[0] == "'=cmd"      # event_id neutralised (no raw leading =)
        assert cells[2] == "'@danger"   # gene_id neutralised

    def test_formula_safe_ids_round_trip(self, tmp_path: Path) -> None:
        """A formula-leading id is written inert but reads back as the original
        value, so machine round-trip (join/key) is preserved."""
        events = [
            ASEvent(
                event_id="=cmd", event_type=EventType.SE, gene_id="@danger",
                chrom="chr1", strand="+",
                coordinates={"upstream_exon_end": 200},
                inclusion_transcripts=["tx1"], exclusion_transcripts=["tx2"],
                inclusion_junctions=[(200, 300)], exclusion_junctions=[(200, 500)],
            ),
        ]
        psi_results = [PSIResult("=cmd", 0.75, 75, 25, 2, 1, 100, 0.65, 0.85)]
        tsv_path = tmp_path / "rt.tsv"
        write_events_tsv(tsv_path, events, psi_results, [0.9])
        row = read_events_tsv(tsv_path)[0]
        assert row["event_id"] == "=cmd"      # restored (not '=cmd)
        assert row["gene_id"] == "@danger"

    def test_ioe_formula_injection_neutralised(self, tmp_path: Path) -> None:
        """The SUPPA2-compatible IOE writer must also neutralise formula leaders."""
        events = [
            ASEvent(
                event_id="=cmd", event_type=EventType.SE, gene_id="@danger",
                chrom="chr1", strand="+",
        ...
        ioe_path = tmp_path / "x.ioe"
        write_ioe(ioe_path, events)
        cells = ioe_path.read_text().splitlines()[1].split("\t")
        assert cells[1] == "'@danger"   # gene_id neutralised
        assert cells[2] == "'=cmd"      # event_id neutralised
