...
class TestFeatureExtraction:
    ...
    def test_unmatched_exon_coverage_does_not_fall_back_to_weight(self) -> None:
        """Partially unresolved exons should not mix flow weight into coverage."""
        graph = SpliceGraph(chrom="chr1", strand="+", locus_start=100, locus_end=700)
        source = graph.add_node(100, 100, NodeType.SOURCE, coverage=0.0)
        exon = graph.add_node(100, 200, NodeType.EXON, coverage=10.0)
        sink = graph.add_node(700, 700, NodeType.SINK, coverage=0.0)
        graph.add_edge(source, exon, EdgeType.SOURCE_LINK, weight=10.0, coverage=10.0)
        graph.add_edge(exon, sink, EdgeType.SINK_LINK, weight=10.0, coverage=10.0)
        tx = Transcript(
            node_ids=[],
        ...
        assert feat.mean_coverage == pytest.approx(10.0)
        assert feat.max_coverage == pytest.approx(10.0)
...
class TestHeuristicScorer:
    ...
    def test_explicit_missing_model_path_raises(self) -> None:
        """An explicit --model path that does not exist must fail fast, not
        silently fall back to a different (heuristic) scorer (ML provenance)."""
        with pytest.raises(FileNotFoundError, match="model file not found"):
            TranscriptScorer("/tmp/__braid_definitely_missing_model__.joblib")
...
class TestScorerTrainAndPredict:
    ...
    def test_oob_metric_is_balanced_accuracy(self) -> None:
        """OOB monitoring uses balanced accuracy (honest for the imbalanced
        true/false-transcript task), not raw accuracy."""
        scorer = TranscriptScorer()
        n_features = scorer.n_features
        rng = np.random.default_rng(0)
        X = rng.uniform(0, 10, size=(200, n_features))
        names = feature_names()
        cov_idx = names.index("coverage_uniformity")
        y = (X[:, cov_idx] > 8.0).astype(np.int64)  # imbalanced (few positives)
        y[0] = 0
        y[1] = 1

        scorer.train(X, y, n_estimators=20, max_depth=5)

        md = scorer._training_metadata
        assert "oob_balanced_accuracy" in md
        assert "oob_score" not in md  # the misleading raw-accuracy key is gone
        assert 0.0 <= md["oob_balanced_accuracy"] <= 1.0

    def test_train_rejects_single_class(self) -> None:
        """A single-class training set must fail fast rather than build a
        degenerate model whose predict_proba has no class-1 column."""
        scorer = TranscriptScorer()
        n_features = scorer.n_features
        rng = np.random.default_rng(1)
        X = rng.uniform(0, 10, size=(30, n_features))
        y = np.zeros(30, dtype=np.int64)  # only class 0

        with pytest.raises(ValueError, match="both classes"):
            scorer.train(X, y, n_estimators=5, max_depth=3)
    ...
    def test_load_rejects_feature_name_schema_drift(self, tmp_path) -> None:
        """A schema-drifted model artifact must not load and score silently."""
        import joblib

        scorer = TranscriptScorer()
        n_features = scorer.n_features
        rng = np.random.default_rng(7)
        X = rng.uniform(0, 10, size=(40, n_features))
        y = rng.integers(0, 2, size=40).astype(np.int64)
        y[0] = 0
        y[1] = 1
        scorer.train(X, y, n_estimators=5, max_depth=3)
        ...
        fresh = TranscriptScorer()
        with pytest.raises(ValueError, match="feature names do not match"):
            fresh.load(str(model_path), allow_untrusted=True)
        assert not fresh.is_trained

    def test_load_refuses_untrusted_model_by_default(self, tmp_path) -> None:
        """joblib.load is arbitrary code execution; load() must refuse without an
        explicit trust opt-in, so `--model attacker.joblib` is not silently run."""
        import joblib
        p = tmp_path / "m.joblib"
        joblib.dump({"model": "not-a-real-model"}, str(p))
        scorer = TranscriptScorer()
        with pytest.raises(RuntimeError, match="Refusing to load"):
            scorer.load(str(p))                       # no opt-in -> refused
        with pytest.raises(ValueError):               # opt-in reaches the type check
            scorer.load(str(p), allow_untrusted=True)
...
class TestRemoveRedundant:
    ...
    def test_overlapping_single_exons_are_not_empty_chain_duplicates(self) -> None:
        """Single-exon transcripts need exact exon identity to be redundant."""
        tx_a = Transcript(exon_coords=[(100, 200)], weight=10.0)
        tx_b = Transcript(exon_coords=[(180, 320)], weight=5.0)
        filt = TranscriptFilter(config=FilterConfig(remove_redundant=True))
    ...
    def test_identical_single_exon_duplicate_removed(self) -> None:
        """Exact duplicate single-exon transcripts still collapse."""
        tx_a = Transcript(exon_coords=[(100, 200)], weight=10.0)
        tx_b = Transcript(exon_coords=[(100, 200)], weight=5.0)
        filt = TranscriptFilter(config=FilterConfig(remove_redundant=True))
    ...
    def test_locus_grouping_does_not_transitively_chain_readthrough(self) -> None:
        """A bridge transcript must not merge two adjacent loci into one group."""
        transcripts = [
            Transcript(exon_coords=[(100, 200)], weight=5.0),
            Transcript(exon_coords=[(180, 320)], weight=9.0),
            Transcript(exon_coords=[(300, 400)], weight=4.0),
        ]

        groups = TranscriptFilter._group_by_overlap(transcripts, [0, 1, 2])

        assert groups == [[0, 1], [2]]

    def test_locus_cap_keeps_second_locus_despite_readthrough_bridge(self) -> None:
        """Per-locus cap should not let a bridge suppress a downstream locus."""
        transcripts = [
            Transcript(exon_coords=[(100, 200)], weight=5.0),
            Transcript(exon_coords=[(180, 320)], weight=9.0),
            Transcript(exon_coords=[(300, 400)], weight=4.0),
        ]
        scores = np.array([0.8, 0.95, 0.7], dtype=np.float64)
        filt = TranscriptFilter(config=FilterConfig(max_transcripts_per_locus=1))

        surviving = filt._apply_locus_cap(transcripts, scores, [0, 1, 2], 1)
...
def test_shannon_entropy_handles_zeros_without_nan() -> None:
    """_shannon_entropy must filter zero-probability bins before log, so a
    distribution with empty bins yields finite entropy, never 0*log(0)=NaN."""
    import numpy as np

    from braid.scoring.features import _shannon_entropy

    h = _shannon_entropy(np.array([3.0, 0.0, 1.0, 0.0]))
    assert np.isfinite(h) and h > 0.0
    # all-zero / empty arrays are defined as 0.0 entropy
    assert _shannon_entropy(np.array([0.0, 0.0])) == 0.0
    assert _shannon_entropy(np.array([])) == 0.0
    # uniform two-bin distribution has entropy log(2)
    assert abs(_shannon_entropy(np.array([1.0, 1.0])) - float(np.log(2))) < 1e-9
