...
def test_bootstrap_confidence_is_deterministic_with_seed() -> None:
    """The default (CPU) bootstrap is reproducible: the same seed yields identical
    intervals. The approximate GPU path that could break this is opt-in/off."""
    graph, paths = _make_simple_graph()
    r1 = bootstrap_confidence(graph, paths, BootstrapConfig(n_replicates=80, seed=123))
    r2 = bootstrap_confidence(graph, paths, BootstrapConfig(n_replicates=80, seed=123))
    assert len(r1.transcripts) == len(r2.transcripts)
    for t1, t2 in zip(r1.transcripts, r2.transcripts):
        assert t1.weight_ci_low == t2.weight_ci_low
        assert t1.weight_ci_high == t2.weight_ci_high
        assert t1.presence_rate == t2.presence_rate


def test_bootstrap_config_gpu_off_by_default() -> None:
    assert BootstrapConfig().allow_approximate_gpu is False


def test_to_csr_carries_edge_types_and_only_introns_are_junctions() -> None:
    """to_csr() propagates per-edge types so the bootstrap resamples only INTRON
    edges -- not source/sink/continuation edges, which the old weight==coverage
    heuristic mis-flagged as junctions."""
    from braid.flow.bootstrap import _identify_junction_edges
    ...
    e2 = g.add_node(start=300, end=400, node_type=NodeType.EXON, coverage=30)
    sink = g.add_node(start=600, end=600, node_type=NodeType.SINK, coverage=0)
    g.add_edge(s, e1, EdgeType.SOURCE_LINK, weight=50, coverage=50)
    g.add_edge(e1, e2, EdgeType.INTRON, weight=30, coverage=30)
    g.add_edge(e2, sink, EdgeType.SINK_LINK, weight=30, coverage=30)

    csr = g.to_csr()
    assert csr.edge_types is not None
    assert len(csr.edge_types) == csr.n_edges

    mask = _identify_junction_edges(csr)
    # exactly one junction (the INTRON edge), not all three
    assert int(mask.sum()) == 1
    intron_idx = list(np.asarray(csr.edge_types)).index(int(EdgeType.INTRON))
    assert mask[intron_idx]
