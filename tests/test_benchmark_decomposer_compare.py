...
def test_variant_specs_support_braid_v2_no_motif_validation() -> None:
    """Synthetic comparison matrix should expose braid_v2 benchmark variants."""
    variants = _variant_specs(["braid_v2_no_motif_validation"], shadow_mode="legacy")

    assert len(variants) == 1
    assert variants[0].decomposer == "braid_v2"
    assert variants[0].enable_motif_validation is False
