...
class TestAddNodesAndEdges:
    ...
    def test_duplicate_intron_edges_accumulate_support(self) -> None:
        """Duplicate resolved introns should sum support instead of overwrite."""
        g = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=500)
        n0 = g.add_node(start=100, end=200, node_type=NodeType.EXON)
        n1 = g.add_node(start=300, end=400, node_type=NodeType.EXON)
        g.add_edge(n0, n1, EdgeType.INTRON, weight=4.0, coverage=4.0)
        g.add_edge(n0, n1, EdgeType.INTRON, weight=6.0, coverage=6.0)

        edge = g.get_edge(n0, n1)
        assert edge is not None
        assert edge.weight == pytest.approx(10.0)
        assert edge.coverage == pytest.approx(10.0)

    def test_single_intron_edge_is_not_self_accumulated(self) -> None:
        """A single INTRON add stores its support verbatim (no spurious doubling)."""
        g = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=500)
        n0 = g.add_node(start=100, end=200, node_type=NodeType.EXON)
        n1 = g.add_node(start=300, end=400, node_type=NodeType.EXON)
        g.add_edge(n0, n1, EdgeType.INTRON, weight=4.0, coverage=4.0)
        ...
        edge = g.get_edge(n0, n1)
        assert edge is not None
        assert edge.weight == pytest.approx(4.0)
        assert edge.coverage == pytest.approx(4.0)
        # Adjacency is registered exactly once for the junction.
        assert g.get_successors(n0).count(n1) == 1
        assert g.get_predecessors(n1).count(n0) == 1

    def test_non_intron_duplicate_edge_replaces_not_accumulates(self) -> None:
        """Only INTRON+INTRON accumulates; other duplicates replace in place."""
        g = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=500)
        n0 = g.add_node(start=0, end=0, node_type=NodeType.SOURCE)
        n1 = g.add_node(start=100, end=200, node_type=NodeType.EXON)
        g.add_edge(n0, n1, EdgeType.SOURCE_LINK, weight=3.0, coverage=3.0)
        g.add_edge(n0, n1, EdgeType.SOURCE_LINK, weight=5.0, coverage=5.0)

        edge = g.get_edge(n0, n1)
        assert edge is not None
        assert edge.weight == pytest.approx(5.0)
        assert edge.coverage == pytest.approx(5.0)
        # In-place replacement must not duplicate the adjacency entry.
        assert g.get_successors(n0).count(n1) == 1
        assert g.get_predecessors(n1).count(n0) == 1

    def test_mixed_type_duplicate_edge_does_not_accumulate(self) -> None:
        """An INTRON edge overwritten by a non-INTRON type replaces, not sums."""
        g = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=500)
        n0 = g.add_node(start=100, end=200, node_type=NodeType.EXON)
        n1 = g.add_node(start=300, end=400, node_type=NodeType.EXON)
        g.add_edge(n0, n1, EdgeType.INTRON, weight=4.0, coverage=4.0)
        g.add_edge(n0, n1, EdgeType.CONTINUATION, weight=6.0, coverage=6.0)

        edge = g.get_edge(n0, n1)
        assert edge is not None
        assert edge.edge_type == EdgeType.CONTINUATION
        assert edge.weight == pytest.approx(6.0)
        assert edge.coverage == pytest.approx(6.0)
...
class TestGraphSimplify:
    ...
    def test_simplify_skips_self_loop_merge(self) -> None:
        """Simplification must not collapse a continuation cycle into a self-loop."""
        g = SpliceGraph(chrom="chr1", strand="+", locus_start=0, locus_end=500)
        a = g.add_node(start=100, end=200, node_type=NodeType.EXON, coverage=3.0)
        b = g.add_node(start=200, end=300, node_type=NodeType.EXON, coverage=3.0)
        g.add_edge(a, b, EdgeType.CONTINUATION, weight=3.0, coverage=3.0)
        g.add_edge(b, a, EdgeType.CONTINUATION, weight=3.0, coverage=3.0)

        g.simplify()

        assert g.get_edge(a, a) is None
        assert g.get_edge(b, b) is None
        assert g.n_nodes == 2
...
class TestBuilderProfiles:
    ...
    def test_junction_merge_does_not_single_link_chain_real_sites(self) -> None:
        """Near-neighbour chains must not collapse sites spanning > merge_dist."""
        starts, ends, counts = _merge_nearby_junctions(
            [100, 110, 120, 130],
            [500, 500, 500, 500],
            [1.0, 10.0, 9.0, 2.0],
            merge_dist=10,
        ...
        assert starts == [110, 120]
        assert ends == [500, 500]
        assert counts == [10.0, 9.0]
    ...
    def test_terminal_boundary_no_coverage_uses_minimum_span(self) -> None:
        """A coverage-free terminal window must not collapse to the splice anchor."""
        read_data = ReadData(
            chrom_ids=np.array([0], dtype=np.int32),
            positions=np.array([0], dtype=np.int64),
            end_positions=np.array([10], dtype=np.int64),
            strands=np.zeros(1, dtype=np.int8),
            mapping_qualities=np.full(1, 60, dtype=np.uint8),
            is_paired=np.zeros(1, dtype=np.bool_),
            is_read1=np.zeros(1, dtype=np.bool_),
            mate_positions=np.full(1, -1, dtype=np.int64),
            mate_chrom_ids=np.full(1, -1, dtype=np.int32),
            cigar_ops=np.array([0], dtype=np.uint8),
            cigar_lens=np.array([10], dtype=np.int32),
            cigar_offsets=np.array([0, 1], dtype=np.int64),
            query_names=["read_0"],
            n_reads=1,
        )

        left = _compute_terminal_boundary(
            read_data, 0, 1000, "left", max_extend=100, min_boundary_span=20,
        )
        right = _compute_terminal_boundary(
            read_data, 0, 1000, "right", max_extend=100, min_boundary_span=20,
        )
