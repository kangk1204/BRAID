...
class TestMutuallyExclusiveExons:
    ...
    def test_mxe_detection_is_input_order_independent(self) -> None:
        """MXE should be detected even when the later exon transcript is first."""
        tx_x = _tx("tx_x", "g1", [(100, 200), (300, 400), (600, 700)])
        tx_y = _tx("tx_y", "g1", [(100, 200), (450, 550), (600, 700)])

        forward = _detect_mutually_exclusive_exons([tx_x, tx_y], "g1")
        reversed_order = _detect_mutually_exclusive_exons([tx_y, tx_x], "g1")

        assert len(forward) == 1
        assert len(reversed_order) == 1
        assert reversed_order[0].event_id == forward[0].event_id
        assert reversed_order[0].inclusion_transcripts == ["tx_x"]
        assert reversed_order[0].exclusion_transcripts == ["tx_y"]
    ...
    def test_no_mxe_bookended_exons(self) -> None:
        """Boundary-touching internal exons are not separated MXE alternatives."""
        tx_a = _tx("tx1", "g1", [(100, 200), (300, 400), (600, 700)])
        tx_b = _tx("tx2", "g1", [(100, 200), (400, 500), (600, 700)])
        events = _detect_mutually_exclusive_exons([tx_a, tx_b], "g1")
        assert len(events) == 0
...
class TestRetainedIntron:
    ...
    def test_basic_ri(self) -> None:
        ...
        assert e.inclusion_transcripts == ["tx1"]
        assert e.exclusion_transcripts == ["tx2"]
        assert e.inclusion_junctions == []
        assert e.exclusion_junctions == [(300, 400)]
