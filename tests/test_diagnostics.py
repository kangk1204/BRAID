...
def test_diagnostics_collector_writes_summary_and_jsonl(tmp_path) -> None:
    ...
    # A fully-aggregated single-process report is flagged complete with no note.
    assert summary["complete"] is True
    assert "note" not in summary


def test_incomplete_note_marks_summary_not_complete(tmp_path) -> None:
    """When diagnostics are not fully aggregated (e.g. parallel workers do not
    return per-chromosome records), the summary must advertise complete=false and
    carry the explanatory note, so a machine reader never mistakes an
    un-aggregated count for a genuine zero."""
    out_dir = tmp_path / "diagnostics"
    collector = DiagnosticsCollector(out_dir)
    collector.incomplete_note = (
        "Per-chromosome diagnostics are not aggregated under --threads >1."
    )
    ...
    assert summary["note"] == collector.incomplete_note
