...
def test_run_target_rejects_gene_and_region_together() -> None:
    ...
    with pytest.raises(SystemExit, match="--gene or --region"):
        _run_target(Namespace(
            bam="sample.bam", gene="TP53", region="chr17:1-100", gtf="ann.gtf",
            reference=None, flank=1000, max_paths=5000, bootstrap_replicates=10,
            min_presence=0.3, strandedness="none", format="text", output=None,
            verbose=False,
        ))
