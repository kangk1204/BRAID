...
import braid.flow.nmf_decompose as nmf_module
...
class TestNMFMultiplicative:
    ...
    def test_seeded_restarts_are_reproducible(self) -> None:
        """Explicit NMF seeds should make multi-restart fits reproducible."""
        X = np.random.default_rng(3).random((8, 5), dtype=np.float32)

        W1, H1, err1 = _nmf_multiplicative(
            X, k=2, max_iter=30, n_restarts=3, random_state=7,
        )
        W2, H2, err2 = _nmf_multiplicative(
            X, k=2, max_iter=30, n_restarts=3, random_state=7,
        )
    ...
    def test_more_restarts_do_not_worsen_reconstruction(self) -> None:
        """Best-of-N restarts should be at least as good as one restart."""
        X = np.random.default_rng(4).random((12, 6), dtype=np.float32)

        _, _, err_one = _nmf_multiplicative(
            X, k=3, max_iter=30, n_restarts=1, random_state=11,
        )
        _, _, err_many = _nmf_multiplicative(
            X, k=3, max_iter=30, n_restarts=4, random_state=11,
        )
...
class TestSelectK:
    ...
    def test_select_k_evaluates_full_candidate_range(
        self, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """BIC-over-K is not guaranteed unimodal, so late minima must be seen."""
        calls: list[int] = []
        errors = {1: 20.0, 2: 20.0, 3: 20.0, 4: 20.0, 5: 1.0}
        ...
        def fake_nmf(
            ...
            calls.append(k)
            return np.empty((X.shape[0], k)), np.empty((k, X.shape[1])), errors[k]

        monkeypatch.setattr(nmf_module, "_nmf_multiplicative", fake_nmf)
        X = np.ones((20, 20), dtype=np.float32)

        k = _select_k(X, max_k=5, max_iter=10)
...
class TestDecomposeNMF:
    ...
    def test_config_defaults(self) -> None:
        ...
        assert cfg.nmf_restarts == 3
        assert cfg.nmf_random_seed == 42
