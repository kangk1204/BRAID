...
class CSRGraph:
    ...
    n_nodes: int
    n_edges: int
    edge_types: np.ndarray | None = None  # int8 EdgeType ordinal per edge
...
class SpliceGraph:
    ...
    def add_edge(
        ...
        If the edge ``(src, dst)`` already exists it is replaced, except
        duplicate INTRON evidence is accumulated as read support.
        ...
        existing = old_edge is not None
        if (
            old_edge is not None
            and old_edge.edge_type == EdgeType.INTRON
            and edge_type == EdgeType.INTRON
        ):
            edge = SpliceEdge(
                src=src,
                dst=dst,
                edge_type=edge_type,
                weight=old_edge.weight + weight,
                coverage=old_edge.coverage + coverage,
            )
        else:
            edge = SpliceEdge(
                src=src,
    ...
    def to_csr(self) -> CSRGraph:
        ...
        out_edges: list[list[tuple[int, float, float, int]]] = [[] for _ in range(n)]
        for (src, dst), edge in self._edges.items():
            new_src = old_to_new[src]
            new_dst = old_to_new[dst]
            out_edges[new_src].append(
                (new_dst, edge.weight, edge.coverage, int(edge.edge_type))
            )
        ...
        edge_types = np.empty(total_edges, dtype=np.int8)
...
class BatchedCSRGraphs:
    ...
    def __init__(self) -> None:
        ...
        self._edge_types: np.ndarray | None = None
    ...
    def finalize(self) -> None:
        ...
        global_edge_types = (
            np.empty(total_edges, dtype=np.int8)
            if all(g.edge_types is not None for g in self._graphs)
            else None
        )
        ...
        for i, g in enumerate(self._graphs):
            ...
            if global_edge_types is not None:
                global_edge_types[es:ee] = g.edge_types
        ...
        self._edge_types = global_edge_types
