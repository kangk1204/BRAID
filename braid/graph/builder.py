...
The algorithm follows established splice graph formulations for transcript
assembly, adapted for batch GPU processing.
"""
...
from braid.utils.interval import compute_coverage
...
def _merge_nearby_junctions(
    ...
    # Cluster by acceptor proximity.  Anchor each cluster to its first
    # coordinate so a chain of near-neighbours cannot span far beyond
    # ``merge_dist`` and collapse real alternative splice sites.
    acc_clusters: list[list[int]] = []
    cur_cluster: list[int] = [indices[0]]
    for k in range(1, n):
        if abs(j_ends[indices[k]] - j_ends[cur_cluster[0]]) <= merge_dist:
    ...
    for cluster in acc_clusters:
        ...
        for k in range(1, len(cluster)):
            if abs(j_starts[cluster[k]] - j_starts[sub[-1][0]]) <= merge_dist:
...
def _compute_terminal_boundary(
    ...
    Args:
        ...
        min_boundary_span: Minimum fallback span if no coverage supports a
            terminal boundary estimate.

    Returns:
        The boundary position.
    """
    fallback_span = max(1, min(int(min_boundary_span), int(max_extend)))

    def fallback_boundary() -> int:
        if direction == "left":
            return max(0, anchor_pos - fallback_span)
        return anchor_pos + fallback_span
