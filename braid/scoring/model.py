...
from braid.scoring.features import feature_names
...
class TranscriptScorer:
    ...
    Args:
        model_path: Optional filesystem path to a ``joblib``-serialized
            model file. If ``None``, the scorer uses the heuristic fallback;
            a path that is given but does not exist raises ``FileNotFoundError``
            (fail-fast on a missing/typo'd model artifact).
    """
    ...
    def __init__(self, model_path: str | None = None) -> None:
        ...
        if model_path is not None:
            # An explicit --model path that does not exist is a provenance error
            # (typo / missing deploy artifact); fail fast rather than silently
            # scoring with a different (heuristic) model. Omit --model for the
            # heuristic default.
            if not Path(model_path).exists():
                raise FileNotFoundError(
                    f"model file not found: {model_path}. Pass an existing --model "
                    "path, or omit --model to use the heuristic scorer."
                )
    ...
    def train(
        ...
        if unique_labels != {0, 1}:
            # A single-class fit produces a degenerate model whose predict_proba
            # has one column, so score()/score_batch() (which read column 1)
            # would crash. Fail fast on the broken training set instead.
            raise ValueError(
                "y must contain both classes (0 and 1) to train a discriminative "
                f"scorer; found only {sorted(unique_labels)}."
            )
        ...
        # The true/false-transcript classes are imbalanced (hence
        # class_weight="balanced"), so the OOB metric must be balanced too: raw
        # accuracy is dominated by the majority class and would report a strong
        # score for a model that never recovers the minority (true) transcripts.
        ...
        self._training_metadata = {
            ...
            "n_estimators": n_estimators,
            "max_depth": max_depth,
            "oob_balanced_accuracy": float(clf.oob_score_),
            "trained_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
            "feature_names": list(self._feature_names),
        }

        logger.info(
            "Training complete. OOB balanced accuracy: %.4f", clf.oob_score_,
        )
    ...
    def load(self, path: str, *, allow_untrusted: bool = False) -> None:
        """Load a trained model from disk.

        ``joblib.load`` unpickles and therefore executes code embedded in the file,
        so loading is refused unless the caller opts in (``allow_untrusted=True`` or
        the ``BRAID_ALLOW_UNTRUSTED_MODEL`` environment variable). This prevents an
        attacker-supplied model from being silently executed.

        Args:
            path: Filesystem path to a ``joblib``-serialized model file.
            allow_untrusted: Acknowledge that *path* is trusted and permit the
                (code-executing) deserialization.

        Raises:
            FileNotFoundError: If *path* does not exist.
            RuntimeError: If loading is not opted in (untrusted-model guard).
        ...
        # SECURITY: joblib.load deserializes (unpickles) and executes arbitrary code
        # embedded in the file BEFORE the isinstance checks below, so those checks are
        # a schema guard, not an RCE defence. Refuse by default; require an explicit
        # opt-in acknowledging the file is trusted, so an attacker-supplied model is
        # not silently executed via `braid assemble --model attacker.joblib`.
        env_opt_in = os.environ.get(
            "BRAID_ALLOW_UNTRUSTED_MODEL", "").strip().lower() in ("1", "true", "yes")
        if not (allow_untrusted or env_opt_in):
            raise RuntimeError(
                f"Refusing to load model {path!r}: joblib.load executes code embedded "
                "in the file (arbitrary code execution) before any validation. Load "
                "only models you trust, then set BRAID_ALLOW_UNTRUSTED_MODEL=1 to "
                "acknowledge the risk."
            )
        logger.warning(
            "Loading scoring model from %s via joblib: this deserializes the file "
            "and executes any code it contains. Load only trusted model files.",
            filepath,
        )
        ...
        if stored_names and stored_names != self._feature_names:
            raise ValueError(
                "Loaded model feature names do not match this BRAID build "
                f"(expected {len(self._feature_names)} names, artifact has "
                f"{len(stored_names)}). Refusing to score with a schema-drifted "
                "model."
            )
