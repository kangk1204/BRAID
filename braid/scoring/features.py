...
def _get_exon_coverages_from_graph(
    ...
    resolved = [cov for cov in covs if cov is not None]
    if not resolved:
        return []
    fill_cov = float(np.median(np.array(resolved, dtype=np.float64)))
    return [cov if cov is not None else fill_cov for cov in covs]
