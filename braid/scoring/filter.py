...
:class:`braid.flow.decompose.Transcript`, which carries ``exon_coords``,
...
class TranscriptFilter:
    ...
    def filter_transcripts_with_diagnostics(
        ...
        Args:
            transcripts: List of candidate transcripts (instances of
                :class:`~braid.flow.decompose.Transcript`).
    ...
    def _remove_redundant_indices(
        ...
        for group_indices in locus_groups:
            ...
            # Compute redundancy keys for each transcript. Single-exon
            # transcripts have no intron chain, so require exact exon identity
            # before treating them as duplicates.
            chain_keys: dict[int, tuple[str, tuple[tuple[int, int], ...]]] = {}
            for i in sorted_indices:
                chain = _intron_chain_key(transcripts[i])
                if chain:
                    chain_keys[i] = ("spliced", chain)
                else:
                    chain_keys[i] = ("single_exon", tuple(transcripts[i].exon_coords))

            # Keep only the highest-scored transcript per unique intron chain
            seen_chains: set[tuple[str, tuple[tuple[int, int], ...]]] = set()
            removed: set[int] = set()
    ...
    def merge_identical_intron_chains(
        ...
        Returns:
            ...
            unique intron chain.
        """
        from braid.flow.decompose import Transcript as TranscriptCls
    ...
    def _group_by_overlap(
        ...
        """Group transcript indices by a conservative same-locus proxy.

        Since the ``Transcript`` dataclass from ``flow.decompose`` does not
        carry an explicit ``locus_id``, this method groups candidates against
        a fixed anchor transcript instead of expanding groups by transitive
        span overlap.  Spliced transcripts must share at least one exact
        intron; single-exon transcripts use direct overlap with the anchor.
        This keeps a read-through transcript from chaining two adjacent genes
        into one artificial locus.
        ...
        Returns:
            List of lists, where each inner list contains indices that match
            the same fixed-anchor locus proxy.
        """
        if not indices:
            return []

        def _tx_span(i: int) -> tuple[int, int]:
            exons = transcripts[i].exon_coords
            if not exons:
                return (0, 0)
            return (exons[0][0], exons[-1][1])

        def _matches_anchor(anchor_idx: int, candidate_idx: int) -> bool:
            anchor_introns = set(_intron_chain_key(transcripts[anchor_idx]))
            candidate_introns = set(_intron_chain_key(transcripts[candidate_idx]))
            if anchor_introns or candidate_introns:
                return bool(anchor_introns & candidate_introns)

            anchor_start, anchor_end = _tx_span(anchor_idx)
            candidate_start, candidate_end = _tx_span(candidate_idx)
            return candidate_start < anchor_end and anchor_start < candidate_end

        sorted_indices = sorted(indices, key=lambda i: (*_tx_span(i), i))

        groups: list[list[int]] = []
        anchors: list[int] = []

        for idx in sorted_indices:
            for group_idx, anchor_idx in enumerate(anchors):
                if _matches_anchor(anchor_idx, idx):
