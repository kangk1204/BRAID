...
def splice_graph_to_features(
    ...
    node_cov_max = (
        max(float(csr_graph.node_coverages.max()), 1.0)
        if csr_graph.node_coverages.size
        else 1.0
    )
    edge_weight_max = (
        max(float(csr_graph.edge_weights.max()), 1.0)
        if csr_graph.edge_weights.size
        else 1.0
    )
    edge_cov_max = (
        max(float(csr_graph.edge_coverages.max()), 1.0)
        if csr_graph.edge_coverages.size
        else 1.0
    )
    ...
    for i in range(n_nodes):
        node_feats[i, 0] = csr_graph.node_coverages[i] / node_cov_max
        node_feats[i, 1] = (
    ...
    for i in range(n_nodes):
        ...
        for e in range(start_idx, end_idx):
            ...
            ef[0] = float(csr_graph.edge_weights[e]) / edge_weight_max
            ef[1] = float(csr_graph.edge_coverages[e]) / edge_cov_max
...
class GNNScorer:
    ...
    def score(
        ...
        if callable(self._fallback_fn):
            fallback_features = (
                graph_data.node_features[mask].mean(axis=0)
                if mask.sum() > 0
                else np.zeros(NODE_FEATURE_DIM, dtype=np.float32)
            )
            return float(np.clip(self._fallback_fn(fallback_features), 0.0, 1.0))
    ...
    def train_model(
        ...
        Args:
            ...
            lr: Learning rate.
            seed: Seed for weight initialisation, so training is reproducible.
