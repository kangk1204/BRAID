...
def _infer_strand_from_orientation(flag: int, is_reverse: bool, strandedness: str) -> int:
    """Infer transcript strand when no XS tag is present."""
    if strandedness == "rf":
        # fr-firststrand/dUTP: paired read1 maps antisense; single-end reads
        # follow the same antisense protocol without inventing read1 status.
        if flag & 0x1 and (flag & (0x40 | 0x80)):
            is_read1 = bool(flag & 0x40)
            return 0 if (is_reverse == is_read1) else 1
        return 0 if is_reverse else 1
    if strandedness == "fr":
        # fr-secondstrand: paired read1 maps sense; single-end reads map sense.
        if flag & 0x1 and (flag & (0x40 | 0x80)):
            is_read1 = bool(flag & 0x40)
            return 1 if (is_reverse == is_read1) else 0
        return 1 if is_reverse else 0
    return 1 if is_reverse else 0
...
class BamReader:
    ...
    Raises:
        ...
        FileNotFoundError: If the alignment index (``.bai``, ``.csi``, or
            ``.crai``) is missing.
    """
    ...
    def __init__(
        ...
        index_candidates = [
            ...
            path.with_suffix(path.suffix + ".crai"),
    ...
    def count_reads(self, chrom: str | None = None, *, approximate: bool = False) -> int:
        """Return the number of reads passing the current flag/quality filters.

        When no filter narrows the set (``min_mapq <= 0`` and no required/filter
        flags), the BAM index statistics give the mapped count without iterating.
        As soon as any filter is active the index ``mapped`` total would
        over-count -- it ignores MAPQ and still counts secondary, supplementary,
        duplicate, and QC-fail records that ``_passes_filters`` rejects -- so we
        iterate and apply the same filters to honour the documented contract.
        Falls back to iteration if index stats are unavailable.

        Args:
            chrom: If given, count only reads on this reference.  Otherwise
                count reads on all references.
            approximate: If True, always use the O(1)-per-contig BAM index
                ``mapped`` total even when filters are active. This is a fast
                upper bound (it over-counts secondary/supplementary/duplicate/
                QC-fail records and ignores MAPQ), not the exact filtered count;
                use only when startup speed on very large BAMs matters more than
                an exact total.

        Returns:
            Number of reads passing the current flag/quality filters (or, when
            ``approximate`` is set, the index mapped total as a fast upper bound).
        """
        filters_active = (
            self._min_mapq > 0
            or self._required_flags != 0
            or self._filter_flags != 0
        )
        with pysam.AlignmentFile(self._bam_path, "rb") as af:
            if filters_active and not approximate:
                # Index stats cannot honour MAPQ/flag filters; count exactly.
    ...
    with pysam.AlignmentFile(bam_path, "rb") as af:
        for read in af.fetch(chrom, region_start, region_end):
            ...
            if read.has_tag("XS"):
                ...
                strand = _infer_strand_from_orientation(
                    flag, read.is_reverse, strandedness,
                )
