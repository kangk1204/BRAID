...
into standard GTF2.2 and GFF3 annotation formats using conventional transcript
and exon feature records. Coordinates are stored internally as 0-based half-open intervals
...
class GtfWriter:
    ...
    Emits conventional GTF transcript assembly records:
    ...
    def _write_transcript_entry(
        ...
        if record.bootstrap_ci_high is not None:
            transcript_attrs += (
                f' bootstrap_ci_high "{record.bootstrap_ci_high:.2f}";'
            )
        if record.bootstrap_presence is not None:
            transcript_attrs += (
                f' bootstrap_presence "{record.bootstrap_presence:.3f}";'
            )
        if record.bootstrap_cv is not None:
            transcript_attrs += f' bootstrap_cv "{record.bootstrap_cv:.3f}";'
