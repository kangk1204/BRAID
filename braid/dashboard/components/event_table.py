...
def _csv_safe_frame(df: "pd.DataFrame") -> "pd.DataFrame":
    """Return a copy of *df* with string cells neutralised against spreadsheet
    formula injection, for safe download as a TSV opened in Excel/LibreOffice.

    The dashboard may render a DataFrame loaded from an arbitrary (possibly
    attacker-influenced) table, so the download is hardened independently of how the
    frame was produced. Numeric columns are untouched.
    """
    ...
    for col in safe.columns:
        ...
        if is_object_dtype(dtype) or is_string_dtype(dtype):
            safe[col] = safe[col].map(
                lambda v: csv_safe(v) if isinstance(v, str) else v
            )
    return safe
...
def render_event_table(events_df: "pd.DataFrame") -> None:
    ...
    with col1:
        ...
        event_types = get_event_type_list(events_df)
    ...
    if selected_types:
        from braid.dashboard.data_loader import filter_by_event_types
        filtered = filter_by_event_types(filtered, selected_types)
    ...
    if gene_search:
        # Treat sidebar input as a literal gene-id fragment, not a regex. Pandas
        # defaults to regex=True, so values like "[" or "(" would otherwise raise
        # re.error and crash the dashboard.
        filtered = filtered[
            filtered["gene_id"].str.contains(
                gene_search, case=False, na=False, regex=False,
            )
        ]
    ...
    if len(filtered) > 0:
        csv_data = _csv_safe_frame(filtered).to_csv(index=False, sep="\t")
        st.download_button(
