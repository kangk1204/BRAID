...
class PipelineConfig:
    ...
    def __post_init__(self) -> None:
        # Fail loud on config knobs that are not wired into the pipeline, so a
        # user cannot silently believe a feature ran when it did not.
        if self.use_gnn_scoring:
            raise NotImplementedError(
                "use_gnn_scoring is not wired into the assembly pipeline; the "
                "GNN scorer (braid.scoring.gnn_model) is offline/experimental "
                "and not consumed here. Remove the flag or use the RandomForest "
                "scorer."
            )
        if self.optimize_filter_thresholds:
            raise NotImplementedError(
                "optimize_filter_thresholds is not wired into the assembly "
                "pipeline; run braid.scoring.auto_filter directly instead."
            )
        for _flag in (
            "use_neural_decomposition",
            "use_junction_scoring",
            "use_boundary_detection",
        ):
            if getattr(self, _flag):
                raise NotImplementedError(
                    f"{_flag} is not wired into the assembly pipeline; the module is "
                    "constructed but never consumed. Remove the flag (consistent with "
                    "use_gnn_scoring), or wire the feature before enabling it."
                )
...
def _make_decompose_config(cfg: PipelineConfig) -> DecomposeConfig:
    """Build the flow-decomposition config from the pipeline config.

    Shared by the single-process method (``_process_chromosome``) and the
    multiprocess worker (``_process_chromosome_worker``) so the two paths cannot
    drift. They previously differed: the worker honoured
    ``cfg.min_relative_abundance`` while the method silently used the
    ``DecomposeConfig`` default, producing different pruning thresholds (and thus
    different transcripts) depending on the thread count.
    """
    return DecomposeConfig(
        ...
        candidate_beam_width=cfg.candidate_beam_width,
...
def _process_chromosome_worker(
    ...
    NOTE: this is a deliberate *reduced-feature* counterpart of the
    single-process method :meth:`AssemblyPipeline._process_chromosome`, NOT stale
    duplication. It intentionally omits the instance-coupled paths the method adds
    — NMF decomposition, the shadow decomposer (``shadow_mode=None`` here), and the
    per-locus diagnostics / stage-timing / thread-safe stats — because those need
    pipeline-instance state that does not exist in a subprocess. The two paths
    MUST stay in parity on the *shared* default-config assembly logic; that
    contract is locked by ``tests/test_pipeline_parallel_parity.py`` (single vs
    parallel produce identical transcripts) and by the shared
    ``_make_builder_config`` / ``_make_decompose_config`` helpers. Edit both paths
    together when changing the shared logic.
    ...
    for locus in loci:
        ...
        decompose_config = _make_decompose_config(cfg)
...
class AssemblyPipeline:
    ...
    def run(self) -> str:
        ...
        try:
            ...
            with Timer("BAM reader initialisation", self._stats):
                ...
                # Accurate filtered total: with the default flag mask this scans
                # the BAM once at startup rather than trusting the index "mapped"
                # total (which over-counts secondary/supplementary). One pass per
                # run; for very large BAMs this adds to startup time by design,
                # trading speed for a correct read count. ``--fast-read-count``
                # (cfg.fast_read_count) opts into the O(1) index total instead.
                if cfg.fast_read_count:
                    logger.warning(
                        "fast_read_count: total_reads uses the BAM index mapped "
                        "total (fast upper bound; over-counts secondary/"
                        "supplementary and ignores MAPQ), not the exact filtered count"
                    )
                self._stats.total_reads = self._bam_reader.count_reads(
                    approximate=cfg.fast_read_count
                )
            ...
            with Timer("Per-chromosome assembly", self._stats):
                go_parallel = cfg.threads > 1 and len(target_chroms) > 1
                # Parallel workers run a simplified decomposition path: NMF and the
                # shadow engine are not applied. A scale flag (--threads) must not
                # silently change the algorithm, so fall back to single-threaded
                # when a semantic flag is set.
                if go_parallel and (cfg.use_nmf_decomposition or cfg.shadow_decomposer):
                    logger.warning(
                        "--nmf/--shadow-engine are not applied by parallel workers; "
                        "running single-threaded to preserve the requested "
                        "decomposition. Drop --nmf/--shadow-engine to use --threads "
                        "for parallel speed."
                    )
                    go_parallel = False
                if go_parallel:
                    # Workers return transcripts only; per-chromosome diagnostics and
                    # assembly summary stats are not aggregated, so flag the gap
                    # rather than letting the dashboard read a misleading zero.
                    logger.warning(
                        "Parallel assembly (--threads %d): per-chromosome diagnostics "
                        "and assembly summary stats are not aggregated across workers "
                        "and will read as zero; use --threads 1 for the full "
                        "diagnostics report.", cfg.threads,
                    )
                    if self._diagnostics is not None:
                        # Flag the diagnostics summary incomplete so the JSON's zero
                        # counts are not mistaken for "nothing processed".
                        self._diagnostics.incomplete_note = (
                            "Per-chromosome diagnostics and assembly summary stats are "
                            "not aggregated under --threads >1; these counts reflect "
                            "only the parent process and are not the processed totals. "
                            "Re-run with --threads 1 for a complete report."
                        )
    ...
    def _process_chromosome(self, chrom: str) -> list[TranscriptRecord]:
        ...
        This is the full-featured single-process path; the parallel subprocess
        counterpart is the module-level :func:`_process_chromosome_worker`, a
        reduced-feature subset (no NMF / shadow decomposer / diagnostics). The two
        MUST stay in parity on the shared default-config assembly logic — locked by
        ``tests/test_pipeline_parallel_parity.py``; edit both together.
    ...
    def _process_chromosomes_parallel(
        ...
        if failed_chroms:
            ...
            raise RuntimeError(
                f"Failed to process {len(failed_chroms)} chromosome(s): {failed}"
            )
