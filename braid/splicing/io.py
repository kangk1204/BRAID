...
def write_events_tsv(
    ...
    with open(output_path, "w", newline="") as fh:
        ...
        for event, score in zip(events, scores):
            ...
            writer.writerow([
                csv_safe(event.event_id),
                EVENT_TYPE_NAMES[event.event_type],
                csv_safe(event.gene_id),
                csv_safe(event.chrom),
                event.strand,
                psi_str,
                inc_count,
                exc_count,
...
def read_events_tsv(input_path: str | Path) -> list[dict[str, str]]:
    ...
    with open(input_path, newline="") as fh:
        ...
        for row in reader:
            # Reverse the formula-injection guard so ids/keys round-trip exactly.
            rows.append({
                k: csv_restore(v) if isinstance(v, str) else v
    ...
    with open(output_path, "w", newline="") as fh:
        ...
        for event in events:
            ...
            writer.writerow([
                ...
                csv_safe(event.gene_id),
                csv_safe(event.event_id),
                csv_safe(",".join(event.inclusion_transcripts)),
                csv_safe(",".join(all_tx)),
            ])
