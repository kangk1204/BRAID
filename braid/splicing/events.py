...
class ASEvent:
    ...
    Attributes:
        ...
        inclusion_transcripts: Transcript IDs supporting the inclusion isoform.
            For RI events, inclusion means the intron-retaining isoform.
        exclusion_transcripts: Transcript IDs supporting the exclusion isoform.
            For RI events, exclusion means the intron-spliced isoform.
        inclusion_junctions: Junction coordinates for inclusion reads. RI
            retention is supported by intron-body evidence, so this list is empty.
        ...
        for j, tx_b in enumerate(transcripts):
            ...
            for ea_idx in range(1, len(tx_a.exons) - 1):
                ...
                for eb_idx in range(1, len(tx_b.exons) - 1):
                    ...
                    # Exons must be separated non-overlapping alternatives.
                    if exon_x == exon_y:
                        continue
                    if max(exon_x[0], exon_y[0]) <= min(exon_x[1], exon_y[1]):
                        continue  # Overlapping or book-ended

                    # Order consistently by genomic coordinate independent of
                    # input transcript order. For MXE, inclusion/exclusion are
                    # deterministic labels for the two alternatives.
                    ...
                    if eid not in events:
                        events[eid] = ASEvent(
                            ...
                            strand=tx_a.strand,
                            coordinates={
                                "upstream_exon_end": upstream[1],
                                "exon_x_start": inc_exon[0],
                                "exon_x_end": inc_exon[1],
                                "exon_y_start": exc_exon[0],
                                "exon_y_end": exc_exon[1],
                                "downstream_exon_start": downstream[0],
                            },
                            inclusion_junctions=[
                                (upstream[1], inc_exon[0]),
                                (inc_exon[1], downstream[0]),
                            ],
                            exclusion_junctions=[
                                (upstream[1], exc_exon[0]),
                                (exc_exon[1], downstream[0]),
                            ],
                        )
