...
class AnnotatedTranscript:
    """Minimal transcript structure for annotation-guided event proposal."""
...
def load_gene_transcripts(
    ...
    """Load annotated transcripts for a gene from a GTF file.

    Args:
        gtf_path: Path to the GTF annotation file.
        gene_name: Gene symbol or gene_id to match, case-insensitive.
        chrom: Optional chromosome filter.

    Returns:
        Sorted list of transcript models for the gene.
    """
    chrom_filter = chrom or ""
    return list(_load_gene_transcripts_cached(
        gtf_path,
        gene_name.upper(),
        chrom_filter,
    ))
...
def _load_gene_transcripts_cached(
    gtf_path: str,
    query: str,
    chrom_filter: str,
) -> tuple[AnnotatedTranscript, ...]:
    """Cached implementation for transcript lookup."""
    ...
    with open(gtf_path, encoding="utf-8") as fh:
        for line in fh:
            ...
            gid = _parse_attr(attrs, "gene_id")
            if (
                (gname is None or gname.upper() != query)
                and (gid is None or gid.upper() != query)
            ):
                continue
            ...
            entry = transcripts.setdefault(tid, {
                "transcript_id": tid,
                "gene_name": gname or query,
                "gene_id": gid,
                "chrom": chrom,
                "strand": fields[6] if fields[6] in ("+", "-") else ".",
                "exons": [],
            })
            entry["exons"].append((int(fields[3]) - 1, int(fields[4])))

    normalized: list[AnnotatedTranscript] = []
    for entry in transcripts.values():
        exons = tuple(sorted(set(entry["exons"])))
        if not exons:
            continue
        normalized.append(AnnotatedTranscript(
    ...
    normalized.sort(key=lambda tx: (tx.chrom, tx.exons[0][0], tx.transcript_id))
