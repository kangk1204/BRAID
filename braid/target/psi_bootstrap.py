...
class BRAIDConfig:
    ...
    calibration_schedule: dict[str, Any] | None = None
...
def classify_support_bin(total_support: float) -> str:
    """Assign one event support total to a named support bin.

    Accepts non-integer totals: the form-length normalization
    (``exc_eff = exc * IncFormLen/SkipFormLen``) makes ``inc + exc_eff`` a float, so a
    total landing in a unit gap between an inclusive ``upper`` (e.g. 19) and the next
    ``lower`` (20) must still bin correctly. The bin is the first whose exclusive upper
    edge (``upper + 1`` == the next bin's lower) exceeds the total, matching
    ``assign_support_bins``' ``np.digitize`` edges; the old ``<= upper`` test dropped
    19 < t < 20 etc. through every bin and returned the least-conservative ``250+``.
    """
    for lower, upper, label in SUPPORT_BIN_LABELS:
        if upper is None:
            return label
        if total_support < upper + 1:
            return label
...
def _resolve_native_calibration_schedule(
    calibration_schedule: dict[str, Any] | None,
    *,
    base_scale: float | None = None,
) -> dict[str, Any]:
...
def sample_psi_posterior(
    ...
    *,
    ...
    if inclusion_count < 0 or exclusion_count < 0:
        raise ValueError(
            "inclusion_count and exclusion_count must be non-negative, got "
            f"inclusion_count={inclusion_count}, exclusion_count={exclusion_count}"
        )
...
def bootstrap_psi(
    ...
    *,
    ...
    """Compute PSI with a confidence interval.

    If ``conformal_calibrator`` is provided, the interval is conformally
    calibrated (``[psi +/- q*std]`` clipped to [0, 1]), giving a distribution-free,
    finite-sample *marginal* coverage guarantee *under exchangeability* of the
    calibration and deployment residuals (not an unconditional or per-event
    guarantee). Otherwise the legacy overdispersed-Beta percentile interval with
    the support-bin inflation schedule is returned.

    ``inc_form_len``/``skip_form_len`` length-normalize the counts to the
    molecular-PSI (rMATS IncLevel) scale: PSI = (inc/IncFormLen) /
    (inc/IncFormLen + exc/SkipFormLen). The exclusion count is rescaled by
    IncFormLen/SkipFormLen so the PSI point estimate and posterior centre match
    rMATS IncLevel and the ``braid differential`` path, while the *raw* read
    totals remain the support/evidence used for conformal binning and the native
    inflation schedule. Both default to 1.0 (identity), so callers that omit them
    ...
    raw_total = inclusion_count + exclusion_count
    if raw_total == 0:
        return 0.0, 0.0, 0.0, float("nan")

    # Length-normalize the exclusion count to the IncLevel scale (identity when
    # form lengths are 1.0). The raw read totals stay the support/evidence.
    ratio = inc_form_len / skip_form_len if skip_form_len > 0 else 1.0
    exc_eff = exclusion_count * ratio
    norm_total = inclusion_count + exc_eff

    psi = inclusion_count / norm_total if norm_total > 0 else 0.0
    ...
    if conformal_calibrator is not None:
        # Distribution-free interval: [psi +/- q*std] clipped to [0, 1]. The
        # posterior std is the per-event uncertainty scale; the calibrator's
        # support-stratified quantile recalibrates it to its nominal marginal
        # coverage (under exchangeability of calibration and deployment residuals).
        # Support bin uses the raw read total, not the length-rescaled one.
        # event_type activates the calibrator's event-type/composite Mondrian quantile
        # when present (q_for cascade); a no-op for support-only calibrators.
        ci_low, ci_high = conformal_calibrator.interval(
            psi, std_psi, raw_total, event_type=event_type
        )
...
def compute_psi_from_junctions(
    ...
    for event in proposed_events:
        ...
        inferred = _infer_event(
            event, evidence, config, rng,
            conformal_calibrator=conformal_calibrator,
        )
