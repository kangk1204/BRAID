...
def _enumerate_all_paths_with_metrics(
    ...
    *,
    ...
    Parameters:
        ...
        max_heap_entries: Maximum number of frontier states retained at once.
            When the frontier exceeds this limit, the lowest-priority
            candidates are pruned rather than dropped in insertion order.
        max_iterations: Hard cap on heap pops to prevent runaway search.
        ...
        for idx in range(start, end):
            ...
            if w > 0:
                ...
                if len(heap) > max_heap_entries:
                    heap = heapq.nsmallest(max_heap_entries, heap)
                    heapq.heapify(heap)
                    frontier_pruned += 1
    ...
    return paths, {
        ...
        "frontier_pruned": frontier_pruned,
    }
...
def solve_nnls_regularized(
    ...
    *,
    ...
    cond_max_dim: int = 512,
) -> tuple[np.ndarray, float]:
    """Non-negative least squares with a conditional Tikhonov ridge.

    Returns ``(weights, condition_number)``. For well-conditioned systems the
    result is bit-identical to :func:`scipy.optimize.nnls`. The path-edge
    incidence matrix is frequently rank-deficient (condition numbers
    ~1e16-1e17), which leaves the NNLS weights in the null space arbitrary.
    When ``cond(A) > cond_threshold`` a tiny ridge (``ridge_rel * ||A||``, added
    by row augmentation) resolves those weights to a stable, reproducible
    minimum-norm solution instead. Conditioning is only measured for matrices
    no larger than ``cond_max_dim`` to bound the SVD cost.
    """
    from scipy.optimize import nnls

    condition_number = float("nan")
    if A.size > 0 and max(A.shape) <= cond_max_dim:
        try:
    ...
    if (
        ...
        and condition_number > cond_threshold
    ):
        n_cols = A.shape[1]
        scale = float(np.linalg.norm(A)) or 1.0
        lam = float(np.sqrt(ridge_rel * scale))
        A_aug = np.vstack([A, lam * np.eye(n_cols, dtype=A.dtype)])
        b_aug = np.concatenate([b, np.zeros(n_cols, dtype=b.dtype)])
        weights, _ = nnls(A_aug, b_aug)
    else:
        weights, _ = nnls(A, b)
    return weights, condition_number
        ...
        for ri, (phase_nodes, phase_count) in enumerate(phasing_paths):
            ...
            for pi, path in enumerate(paths):
                ...
                if _contains_subsequence(path, phase_nodes):
                    # Weight the constraint by scaling BOTH the coefficient and the
                    # target (residual = phasing_weight^2 * (sum(w) - phase_count)^2),
                    # so the row pulls matching paths toward phase_count. Scaling only
                    # the target would instead pull them toward phase_count *
                    # phasing_weight, i.e. lower the weight on matching evidence.
                    A_phasing[ri, pi] = phasing_weight
                    row_has_match = True
            if row_has_match:
...
def decompose_batched(
    ...
    for graph_idx in range(batched.n_graphs):
        ...
        if hasattr(batched, '_edge_types') and batched._edge_types is not None:
            local_csr.edge_types = batched._edge_types[edge_start:edge_end].copy()
