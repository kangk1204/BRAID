...
class BootstrapConfig:
    ...
    Attributes:
        ...
        allow_approximate_gpu: Opt in to the approximate GPU NNLS bootstrap.
            Off by default -- the GPU path uses an approximate (Cholesky+clamp)
            solve that diverges from the exact CPU NNLS, so the default is the
            reproducible CPU path.
    """
    ...
    allow_approximate_gpu: bool = False
...
def _identify_junction_edges(graph_csr: CSRGraph) -> np.ndarray:
    """Return a boolean mask of which edges are junction (intron) edges.

    Prefers the authoritative per-edge type (``EdgeType.INTRON``) when the CSR
    graph carries it. Falls back to the weight==coverage heuristic only for
    legacy graphs without edge types -- that heuristic flags every edge whose
    weight equals its coverage (in practice nearly all edges), which would
    over-resample source/sink/continuation edges and disable the continuation
    flow-balance scaling.
    """
    n_edges = graph_csr.n_edges
    edge_types = getattr(graph_csr, "edge_types", None)
    if edge_types is not None and len(edge_types) == n_edges:
        return np.asarray(edge_types) == int(EdgeType.INTRON)

    is_junction = np.zeros(n_edges, dtype=bool)
    if getattr(graph_csr, "edge_coverages", None) is not None:
...
def _try_gpu_bootstrap(
    ...
    # Derive a deterministic torch RNG seed from the NumPy generator so the
    # GPU path matches CPU reproducibility when config.seed is set.
    torch_seed = int(rng.integers(0, np.iinfo(np.int32).max, dtype=np.int64))
    torch_rng = torch.Generator(device=device)
    torch_rng.manual_seed(torch_seed)
    ...
    poisson_samples = torch.poisson(lambdas, generator=torch_rng)
    resampled[junction_expanded] = poisson_samples[junction_expanded]
    resampled[junction_expanded] = torch.clamp(
        resampled[junction_expanded], min=1.0,
    )
    resampled[~junction_expanded] = torch.clamp(
        resampled[~junction_expanded], min=0.1,
    )
...
def bootstrap_confidence(
    ...
    failed = np.zeros(config.n_replicates, dtype=bool)

    # The GPU path is an APPROXIMATE NNLS (Cholesky + clamp) that diverges from
    # the exact CPU scipy.optimize.nnls, so its results depend on hardware. It
    # is opt-in (config.allow_approximate_gpu) to keep the default intervals
    # exact and reproducible across machines.
    if config.allow_approximate_gpu and _try_gpu_bootstrap(
        ...
        for rep in range(config.n_replicates):
            ...
            try:
                ...
                failed[rep] = True
                logger.debug("Bootstrap replicate %d NNLS failed: %s", rep, exc)

    n_failed = int(failed.sum())
    if n_failed > 0:
        fail_pct = 100.0 * n_failed / config.n_replicates
        (logger.warning if fail_pct >= 5.0 else logger.debug)(
            "Bootstrap: %d/%d replicates failed NNLS (%.1f%%)%s",
            n_failed,
            config.n_replicates,
            fail_pct,
            " — CI may be unreliable." if fail_pct >= 5.0 else "",
        )

    # Compute per-transcript statistics using only successful replicates.
    # Including zero-filled failed rows would bias presence_rate, mean, and CI
    # toward zero without reflecting true sampling uncertainty.
    valid_matrix = weight_matrix[~failed] if n_failed > 0 else weight_matrix
    n_valid = len(valid_matrix)
    ...
    for pi in range(n_paths):
        if n_valid == 0:
            transcripts.append(
                TranscriptConfidence(
                    ...
                    weight_ci_low=float("nan"),
                    weight_ci_high=float("nan"),
                    presence_rate=float("nan"),
                    cv=float("nan"),
                    weights=np.empty(0),
                )
            )
        ...
        col = valid_matrix[:, pi]
        presence = float(np.sum(col > 0) / n_valid)
    ...
    logger.info(
        "Bootstrap: %d/%d replicates succeeded, %d/%d transcripts stable (presence >= %.0f%%)",
        n_valid,
        config.n_replicates,
        n_stable,
        n_paths,
