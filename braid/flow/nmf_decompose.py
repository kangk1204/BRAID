...
class NMFDecomposeConfig:
    ...
    Attributes:
        ...
        nmf_restarts: Number of random NMF initialisations to try.
        nmf_random_seed: Seed for NMF initialisation/subsampling; ``None`` for nondeterministic.
...
def _nmf_multiplicative(
    ...
    Args:
        ...
        n_restarts: Number of random initialisations; best reconstruction wins.
        random_state: Seed for deterministic initialisation; ``None`` for nondeterministic.
    ...
    for _ in range(n_restarts):
        init_seed = int(seed_rng.integers(0, 2**32 - 1))
        rng = np.random.default_rng(init_seed)
        W = rng.random((m, k), dtype=np.float64) + eps
        H = rng.random((k, n), dtype=np.float64) + eps
        ...
        for iteration in range(max_iter):
            ...
            if iteration % 10 == 0:
                ...
                denom = max(prev_err, eps) if np.isfinite(prev_err) else max(err, eps)
...
def _select_k(
    ...
    Args:
        ...
        random_state: Seed for deterministic model selection.
    ...
    for k in range(1, min(max_k + 1, min(m, n) + 1)):
        k_seed = int(seed_rng.integers(0, 2**32 - 1))
        _, _, err = _nmf_multiplicative(
            X, k, max_iter=max_iter,
            n_restarts=n_restarts, random_state=k_seed,
        )
        mse = (err**2) / mn + 1e-10
        n_params = k * (m + n)
    ...
    if n_frags >= 2 and len(read_positions) > 0:
        ...
        if n_reads > 0:
            ...
            if n_reads > max_reads_nmf:
                rng = np.random.default_rng(config.nmf_random_seed)
            ...
            if max_k > 1:
                k = _select_k(
                    X_nmf, max_k, max_iter=50,
                    n_restarts=config.nmf_restarts,
