...
def _identify_safe_edges(
    ...
    if n_edges == 0:
        return safe
    if n_edges > max_edge_probes:
        logger.warning(
            "Skipping safe-edge LP probing for %d edges (limit %d).",
            n_edges, max_edge_probes,
        )
        return safe
    ...
    A_eq = np.zeros((n_conservation, n_edges), dtype=np.float64)
    b_eq = np.zeros(n_conservation, dtype=np.float64)
    ...
    objective_row = -edge_weights.astype(np.float64)[None, :]
    objective_tol = max(1e-7, abs(opt_obj) * 1e-9)
    objective_bound = np.array([opt_obj + objective_tol], dtype=np.float64)

    bounds = [(0.0, max(float(edge_weights[e]), 0.0)) for e in range(n_edges)]
    a_eq = A_eq if n_conservation else None
    b_eq_arg = b_eq if n_conservation else None
...
def compute_safe_paths(
    ...
    # Only positive-flow safe edges define coverage. Always-zero / zero-capacity
    # "safe" edges (flow ~ 0) carry no flow; including them splits a single flow
    # path into segments whose per-chain min-flows are then summed, inflating
    # coverage above 1. Restrict the topology to flow-carrying safe edges.
    safe_mask = safe_mask & (flows > 1e-12)
    ...
    for chain in chains:
        # A safe path must be a simple chain of >= 2 distinct nodes; reject
        # degenerate chains (e.g. a self-loop node->node) that would otherwise
        # contribute a spurious weight.
    ...
    # Coverage is a fraction of total flow, bounded by [0, 1] by definition; clamp
    # as a hard invariant guard against any residual double-counting.
    coverage_fraction = (
        min(1.0, safe_flow / total_flow) if total_flow > 1e-12 else 0.0
    )
